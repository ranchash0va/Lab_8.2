...
public class Main {
        ...
        for (int k = 0; k < countWorker; k++) {
            if (worker[k].Position.equals("Engineer")) {
                ...
                try {
                    ...
                    writer.println("Enter amount of workers :"+"\n"+"Surname: " +worker[k].Surname+"\n"+
                            "Year of birth: " + worker[k].Year+"\n"+"Position: " + worker[k].Position
                            +"\n"+"Salary: " + worker[k].Salary+"\n"+"Education: " + worker[k].Education
                            +"\n"+"Engineers count:" + count);
                    writer.flush();
                    writer.close();
                } catch (
                        IOException ex) {
    ...
    public static void task2(){
        ...
        for (int k = 0; k < countcar; k++) {
            ...
            try {
                PrintWriter writer = new PrintWriter(new BufferedWriter(new FileWriter(myFile)));
                writer.println("Company: " + car[k].Company+"\n"+"Manufacturer: " + car[k].Manufacturer
                        +"\n"+"Car type: " + car[k].Type+"\n"+"Year of assembly: " + car[k].Yearassembly
                        +"\n"+"Year of registration: " + car[k].Year +"\n"+"Month of registration: " + car[k].Month
                        +"\n"+"Day of registration: " + car[k].Day);
                writer.flush();
                writer.close();
            } catch (
                    IOException ex) {
